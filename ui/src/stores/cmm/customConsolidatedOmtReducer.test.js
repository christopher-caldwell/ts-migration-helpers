import reducer from './customConsolidatedOmtReducer';
...
describe('CMM Reducer', () => {
    describe('Custom Consolidated Omt', () => {
        const initialState = {
            loading: false,
            error: null,
            data: {},
        ...
        it('set state as loading when action type is CC_OMT_PENDING', async () => {
            const state = reducer(initialState, { type: CC_OMT_PENDING });
            expect(state).toEqual({ ...initialState, loading: true });
        });

        it('set state with payload when action type is CC_OMT_SUCCESS', async () => {
            const state = reducer(initialState, { type: CC_OMT_SUCCESS, payload: { data: cmmDataMock } });
            expect(state).toEqual({ ...initialState, loading: false, data: cmmDataMock });
        });

        it('set state with error content when action type is CC_OMT_FAILURE', async () => {
