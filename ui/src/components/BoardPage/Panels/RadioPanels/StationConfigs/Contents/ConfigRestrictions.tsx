...
import {
    createHourRestrictions,
    updateHourRestrictions,
} from 'stores/restrictions/restrictionsActions';
...
class ConfigRestrictions extends React.Component {
    constructor(props) {
        ...
        this.RESTRICTION_NAME_REQUIRED = `To create a new hour restriction,
            please name the hour restriction and select at least one hour for the restriction.`;
        this.SYNCHRONIZED_CAN_EDIT = 'Only restrictions synchronized with GSelector can be edited.';
    }
    ...
    handleRestrictionName = e => {
        ...
        const restrictionNameExists =
            value !== '' && restrictions.some(restriction => restriction.name === value);
    ...
    handleRestriction = item => {
        ...
        const restrictionToBeUpdated = restrictions.find(
            restriction => restriction.id === restrictionTemplate.id
        );
        ...
        if (!item.selected) {
            ...
            newRestriction.restrictionHour = newRestriction.restrictionHour.filter(
                i => i.hour !== item.hour
            );
        }
    ...
    stagedData = restrictionId => {
        ...
        return restrictionsChanges.some(
            song =>
                song.getChanges.actualChanges.restriction_id === restrictionId ||
                song.getChanges.previousChanges.restriction_id === restrictionId ||
                (!song.getChanges.actualChanges.restriction_id &&
                    Object.prototype.hasOwnProperty.call(
                        song.getChanges.actualChanges,
                        'restriction_id'
                    ) &&
                    song.getChanges.previousChanges.restriction_id === restrictionId)
        );
    ...
    buildHourRestrictionsElement = () => {
        ...
        const hourRestrictionElement = restrictions.map(hourRestriction => {
            ...
            const syncStatusData = getSyncStatus(
                restrictionSynchronized,
                this.stagedData(restrictionId)
            );
            ...
            let restrictionObj = stagedRestrictions.find(
                currentItem => currentItem.id === hourRestriction.id
            );
            if (!restrictionObj) {
            ...
            return (
                <tr key={key}>
                    ...
                    <td className="station-configs-restrictions__table-column--actions">
                        ...
                        {restrictionSynchronized && (
                            <TextButton
                                onClick={() => this.handleOpenModal(restrictionObj)}
                                text="Edit"
                            />
                        )}
                    </td>
                    <td className="station-configs-restrictions__table-column--actions">
                        ...
                        {restrictionSynchronized && (
                            <TextButton
                                onClick={() => this.openEditSongsRestriction(hourRestriction)}
                                text="Edit"
                            />
                        )}
                    </td>
    ...
    buildAsideModal = () => {
        ...
        return (
            <AsideModal
                title={FEATURE_TITLE.HOUR_RESTRICTION}
                asideModalOpened={editSongsRestrictionOpened}
                handleClose={this.handleCloseAsideModal}
            >
                <AsideModalPanels
                    featureName={FEATURES.HOUR_RESTRICTION}
                    restriction={restrictionToEdit}
                    handleClose={this.handleCloseAsideModal}
                    versions={versions}
    ...
    handleMultiselectHours = (firstChild, isRow = false) => {
        ...
        const everyHourHasRestriction = hours.every(hour =>
            newRestriction.restrictionHour.map(restriction => restriction.hour).includes(hour)
        );
        ...
        if (everyHourHasRestriction) {
            ...
            newRestriction.restrictionHour = unionBy(
                newRestriction.restrictionHour,
                hoursToCheck,
                'hour'
            );
    ...
    buildModal = () => {
        ...
        const tooltipMessage =
            restrictionTemplate.restrictionHour.length === 0 || restrictionTemplate.name === ''
                ? this.RESTRICTION_NAME_REQUIRED
                : '';
        const titleModal =
            restrictionAction === 'create' ? 'Create Hour Restriction' : 'Edit Hour Restriction';

        return (
            <CustomModal
                title={titleModal}
                onClose={this.handleCloseModal}
                onSave={() => this.handleSaveRestriction(restrictionAction)}
            ...
            >
                <div className="station-configs-restrictions-modal__form">
                    {restrictionAction === 'create' ? (
                        <input
                            ...
                            onChange={this.handleRestrictionName}
                            maxLength="50"
                        />
                    ) : (
                        <div className="station-configs-restrictions-modal">
                            <p className="station-configs-restrictions-modal__label">
                                Restriction name:
                            </p>
                            <p
                                className="station-configs-restrictions-modal__name"
                                title={restrictionTemplate.name}
                            >
                                {restrictionTemplate.name}
                            </p>
                        </div>
                ...
                <RestrictionTable
                    restrictionItem={restrictionTemplate}
                    onHourClick={this.handleRestriction}
                    handleMultiselectHours={this.handleMultiselectHours}
                />
            </CustomModal>
        );
    };
    ...
    render() {
        ...
        return (
            <div
                className={classNames('station-configs-restrictions', {
